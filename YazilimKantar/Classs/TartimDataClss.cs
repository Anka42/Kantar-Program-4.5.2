...
namespace YazilimKantar.Classs
{
    class TartimDataClss
    {
        ...
        public DataTable TabloyuGetir()
        {
            sqlcontrol();
            conn.Open();
            SqlDataAdapter adapter = new SqlDataAdapter("select Id,TartimNo,PlakaNo,GiTarih,CiTarih,GiSaat,CiSaat,FirmAdi,GiFirmAdi,Malzeme,Operator,Gelil,Gitil,Aciklama,Tartim,Brut,Kg from Tartim", conn);
        ...
        public void SatiriSil(string ID)
        {
            sqlcontrol();
            conn.Open();
            SqlCommand kmt = new SqlCommand("DELETE  Tartim where Id='" + ID + "'", conn);
            kmt.ExecuteNonQuery();
            conn.Close();
        }
        ...
        public void YeniSatirEkle(string TartimNo, string PlakaNo, string GiTarih, string CiTarih, string GiSaat, string CiSaat, string FirmAdi, string GiFirmAdi, string Malzeme, string Operator, string Gelil, string Gitil, string Aciklama, string Tartim, string Brut, string Kg)
        {
            sqlcontrol();
            conn.Open();
            SqlCommand kmta = new SqlCommand("Insert into  Tartim (TartimNo,PlakaNo,GiTarih,CiTarih,GiSaat,CiSaat,FirmAdi,GiFirmAdi,Malzeme,Operator,Gelil,Gitil,Aciklama,Tartim,Brut,Kg) Values ('" + TartimNo + "','" + PlakaNo + "','" + GiTarih + "','" + CiTarih + "','" + GiSaat + "','" + CiSaat + "','" + FirmAdi + "','" + GiFirmAdi + "','" + Malzeme + "','" + Operator + "','" + Gelil + "','" + Gitil + "','" + Aciklama + "','" + Tartim + "','" + Brut + "','" + Kg + "')", conn);
